...
double BehaviorPlannerFSM::get_look_ahead_distance(const State& ego_state) {
  ...
  LOG(INFO) << "SUNE Calculated look_ahead_distance: " << look_ahead_distance;
...
State BehaviorPlannerFSM::state_transition(const State& ego_state, State goal,
  ...
  if (_active_maneuver == FOLLOW_LANE) {
    ...
    if (is_goal_in_junction) {
      ...
      // DONE: TODO-goal speed at stopping point: What should be the goal speed??
      goal.velocity.x = 0.0;  
      goal.velocity.y = 0.0;  
      goal.velocity.z = 0.0;  
    ...
    // DONE TODO: TODO-maintain the same goal when in STOPPED state: Make sure the new goal
    ...
    if (stopped_secs >= _req_stop_time && tl_state.compare("Red") != 0) {
      // DONE: TODO-move to FOLLOW_LANE state: What state do we want to move to, when
